...
import { IconCalendar, IconPlus, IconChevronLeft, IconChevronRight, IconSearch, IconBell, IconNavPeople, IconNavChart, IconNavGear } from './agenda/ui'
import {
      ...
      <div className="agenda-panel" style={{ position:'fixed', inset:0, zIndex:150, display:'flex', height:'100vh', overflow:'hidden', background:T.bg, fontFamily:T.sans, animation:'agendaPanelIn 300ms ease', WebkitFontSmoothing:'antialiased' }}>
        ...
        <aside style={{ width:252, minWidth:252, background:T.surface, borderRight:`1px solid ${T.line}`, display:'flex', flexDirection:'column', gap:22, padding:'18px 14px', overflowY:'auto' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
            ...
            <button style={navLinkStyle(activeView === 'agenda')} onClick={() => setActiveView('agenda')}>
              <IconCalendar />Agenda
              {activeView === 'agenda' && <span style={{ width:6, height:6, borderRadius:'50%', background:T.accent, marginLeft:'auto', flexShrink:0 }} />}
            </button>
            <button style={navLinkStyle(activeView === 'patients')} onClick={() => setActiveView('patients')}><IconNavPeople />Pacientes</button>
            <button style={navLinkStyle(activeView === 'reports')} onClick={() => setActiveView('reports')}><IconNavChart />Relatórios</button>
          ...
          <div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:7 }}>
              ...
              <div style={statPillStyle}><div><div style={{ fontSize:20, fontWeight:700, fontFamily:T.serif, color:'#b09060', lineHeight:1 }}>{pendingWk}</div><div style={{ fontSize:10, color:T.textMuted, marginTop:2, lineHeight:1.3 }}>Aguardando</div></div></div>
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
          ...
          <div style={{ height:56, flexShrink:0, borderBottom:`1px solid ${T.line}`, background:T.surface, display:'flex', alignItems:'center', padding:'0 20px', gap:14 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, justifyContent:'flex-end' }}>
              ...
              {/* Inbox bell */}
              <div style={{ position:'relative' }}>
                <button onClick={() => setShowInbox(v => !v)} style={{ background:'none', border:'none', cursor:'pointer', color: showInbox ? T.accent : T.textMuted, display:'flex', position:'relative', padding:4 }}>
                  <IconBell />
                  {pending.length > 0 && (
                    <span style={{ position:'absolute', top:-3, right:-4, minWidth:15, height:15, padding:'0 3px', borderRadius:999, background:T.accent, color:'white', fontSize:9, fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', border:`1.5px solid ${T.surface}`, boxSizing:'border-box' }}>
                      {pending.length}
                    </span>
                  )}
                </button>
